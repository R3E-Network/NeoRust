...
use neo::{
	...
	prelude::{JsonRpcClient, NeoBlock},
};
use std::sync::Arc;
...
impl JsonRpc2_Rx {
	...
	pub fn block_index_stream(
		&self,
		polling_interval: Duration,
	) -> impl Stream<Item = Result<u32, Box<dyn std::error::Error>>> {
		stream! {
	...
	pub fn block_stream(
		...
		polling_interval: Duration,
	) -> impl Stream<Item = Result<NeoBlock, Box<dyn std::error::Error>>> {
		self.block_index_stream(polling_interval).then(move |result| async move {
			match result {
				Ok(index) => self
					.provider
					.as_ref()
					.get_block_by_index(index, full_transaction_objects)
					.await
	...
	pub fn catch_up_to_latest_block_stream<S>(
		&self,
		start_block: u32,
		full_transaction_objects: bool,
		on_caught_up_stream: S,
	) -> impl Stream<Item = Result<NeoBlock, Box<dyn std::error::Error>>>
	where
		S: Stream<Item = Result<NeoBlock, Box<dyn std::error::Error>>> + Send + 'static,
	{
		stream! {
	...
	pub fn catch_up_to_latest_and_subscribe_to_new_blocks_stream(
		&self,
		start_block: u32,
		full_transaction_objects: bool,
		polling_interval: Duration,
	...
	pub fn latest_block_index_stream(
		&self,
	) -> impl Stream<Item = Result<u32, Box<dyn std::error::Error>>> {
		stream! {
			match self.provider.as_ref().get_block_count().await {
...
#[cfg(test)]
mod tests {
	use crate::neo_clients::rx::json_rpc_rx::JsonRpc2_Rx;
