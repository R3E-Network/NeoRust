...
/// [`APIError`] is a companion trait to [`crate::Middleware`]. It
...
pub(crate) trait APIError: Error + Sized + Send + Sync {
...
pub trait APITrait: Sync + Send + Debug {
	...
	type Error: APIError<Inner = <<Self as APITrait>::Inner as APITrait>::Error>;
	...
	async fn get_best_block_hash(&self) -> Result<H256, Self::Error> {
		self.get_best_block_hash().await.map_err(APIError::from_err)
	}

	async fn get_block_hash(&self, block_index: u32) -> Result<H256, Self::Error> {
		self.get_block_hash(block_index).await.map_err(APIError::from_err)
	}

	async fn get_block(&self, block_hash: H256, full_tx: bool) -> Result<NeoBlock, Self::Error> {
		self.get_block(block_hash, full_tx).await.map_err(APIError::from_err)
	}

	async fn get_raw_block(&self, block_hash: H256) -> Result<String, Self::Error> {
		self.get_raw_block(block_hash).await.map_err(APIError::from_err)
	}

	// Node methods
	async fn get_block_header_count(&self) -> Result<u32, Self::Error> {
		self.get_block_count().await.map_err(APIError::from_err)
	}
	...
	async fn find_storage(
		...
		self.find_storage(contract_hash, prefix_hex_string, start_index)
			.await
	...
	async fn find_storage_with_id(
		...
		self.find_storage_with_id(contract_id, prefix_hex_string, start_index)
			.await
			.map_err(APIError::from_err)
	}
	// Blockchain methods
	...
	async fn get_transaction_height(&self, tx_hash: H256) -> Result<u32, Self::Error> {
		self.get_transaction_height(tx_hash).await.map_err(APIError::from_err)
	}

	async fn get_next_block_validators(&self) -> Result<Vec<Validator>, Self::Error> {
		self.get_next_block_validators().await.map_err(APIError::from_err)
	}

	async fn get_committee(&self) -> Result<Vec<String>, Self::Error> {
		self.get_committee().await.map_err(APIError::from_err)
	}

	async fn get_connection_count(&self) -> Result<u32, Self::Error> {
		self.get_connection_count().await.map_err(APIError::from_err)
	}

	async fn get_peers(&self) -> Result<Peers, Self::Error> {
		self.get_peers().await.map_err(APIError::from_err)
	}

	// Smart contract method
	async fn get_version(&self) -> Result<NeoVersion, Self::Error> {
		self.get_version().await.map_err(APIError::from_err)
	}

	async fn send_raw_transaction(&self, hex: String) -> Result<RawTransaction, Self::Error> {
		self.send_raw_transaction(hex).await.map_err(APIError::from_err)
	}

	async fn submit_block(&self, hex: String) -> Result<SubmitBlock, Self::Error> {
		self.submit_block(hex).await.map_err(APIError::from_err)
	}
	...
	async fn invoke_function(
		...
		self.invoke_function(contract_hash, method, params, signers)
			.await
			.map_err(APIError::from_err)
	}
	...
	async fn invoke_script(
		...
		self.invoke_script(hex, signers).await.map_err(APIError::from_err)
	}
	...
	async fn calculate_network_fee(&self, hex: String) -> Result<i64, Self::Error> {
		self.calculate_network_fee(hex).await.map_err(APIError::from_err)
	}

	async fn list_address(&self) -> Result<Vec<NeoAddress>, Self::Error> {
		self.list_address().await.map_err(APIError::from_err)
	}
	async fn open_wallet(&self, path: String, password: String) -> Result<bool, Self::Error> {
		self.open_wallet(path, password).await.map_err(APIError::from_err)
	}
	...
	async fn send_from(
		...
		self.send_from(token_hash, from, to, amount).await.map_err(APIError::from_err)
	}
	...
	async fn send_many(
		...
		self.send_many(from, send_tokens).await.map_err(APIError::from_err)
	}
	...
	async fn send_to_address(
		...
		self.send_to_address(token_hash, to, amount).await.map_err(APIError::from_err)
	}

	async fn get_application_log(&self, tx_hash: H256) -> Result<ApplicationLog, Self::Error> {
		self.get_application_log(tx_hash).await.map_err(APIError::from_err)
	}

	async fn get_nep17_balances(&self, script_hash: H160) -> Result<Nep17Balances, Self::Error> {
		self.get_nep17_balances(script_hash).await.map_err(APIError::from_err)
	}

	async fn get_nep17_transfers(&self, script_hash: H160) -> Result<Nep17Transfers, Self::Error> {
		self.get_nep17_transfers(script_hash).await.map_err(APIError::from_err)
	}
	...
	async fn get_nep17_transfers_from(
		...
		self.get_nep17_transfers_from(script_hash, from)
			.await
			.map_err(APIError::from_err)
	}
	...
	async fn get_nep17_transfers_range(
		...
		self.get_nep17_transfers_range(script_hash, from, to)
			.await
			.map_err(APIError::from_err)
	}

	async fn get_nep11_balances(&self, script_hash: H160) -> Result<Nep11Balances, Self::Error> {
		self.get_nep11_balances(script_hash).await.map_err(APIError::from_err)
	}

	// NEP-11 methods

	async fn get_nep11_transfers(&self, script_hash: H160) -> Result<Nep11Transfers, Self::Error> {
		self.get_nep11_transfers(script_hash).await.map_err(APIError::from_err)
	}
	...
	async fn get_nep11_transfers_from(
		...
		self.get_nep11_transfers_from(script_hash, from)
			.await
			.map_err(APIError::from_err)
	}
	...
	async fn get_nep11_transfers_range(
		...
		self.get_nep11_transfers_range(script_hash, from, to)
			.await
			.map_err(APIError::from_err)
	}
	...
	async fn get_nep11_properties(
		...
		self.get_nep11_properties(script_hash, token_id)
			.await
			.map_err(APIError::from_err)
	}

	async fn get_state_root(&self, block_index: u32) -> Result<StateRoot, Self::Error> {
		self.get_state_root(block_index).await.map_err(APIError::from_err)
	}
	...
	async fn get_proof(
		...
		self.get_proof(root_hash, contract_hash, key).await.map_err(APIError::from_err)
	}

	async fn verify_proof(&self, root_hash: H256, proof: &str) -> Result<bool, Self::Error> {
		self.verify_proof(root_hash, proof).await.map_err(APIError::from_err)
	}

	async fn get_state_height(&self) -> Result<StateHeight, Self::Error> {
		self.get_state_height().await.map_err(APIError::from_err)
	}
	...
	async fn get_state(
		...
		self.get_state(root_hash, contract_hash, key).await.map_err(APIError::from_err)
	}
	...
	async fn find_states(
		...
		self.find_states(root_hash, contract_hash, key_prefix, start_key, count)
			.await
			.map_err(APIError::from_err)
	}

	async fn get_block_by_index(&self, index: u32, full_tx: bool) -> Result<NeoBlock, Self::Error> {
		self.get_block_by_index(index, full_tx).await.map_err(APIError::from_err)
	}

	async fn get_raw_block_by_index(&self, index: u32) -> Result<String, Self::Error> {
		self.get_raw_block_by_index(index).await.map_err(APIError::from_err)
	}
	...
	async fn invoke_function_diagnostics(
		...
		self.invoke_function_diagnostics(contract_hash, name, params, signers)
			.await
			.map_err(APIError::from_err)
	}
	...
	async fn invoke_script_diagnostics(
		...
		self.invoke_script_diagnostics(hex, signers).await.map_err(APIError::from_err)
	}
	...
	async fn traverse_iterator(
		...
		self.traverse_iterator(session_id, iterator_id, count)
			.await
			.map_err(APIError::from_err)
	}

	async fn terminate_session(&self, session_id: &str) -> Result<bool, Self::Error> {
		self.terminate_session(session_id).await.map_err(APIError::from_err)
	}
	...
	async fn invoke_contract_verify(
		...
		self.invoke_contract_verify(hash, params, signers)
			.await
			.map_err(APIError::from_err)
	}

	async fn get_raw_mempool(&self) -> Result<MemPoolDetails, Self::Error> {
		self.get_raw_mempool().await.map_err(APIError::from_err)
	}

	async fn import_private_key(&self, wif: String) -> Result<NeoAddress, Self::Error> {
		self.import_private_key(wif).await.map_err(APIError::from_err)
	}

	async fn get_block_header_hash(&self, hash: H256) -> Result<NeoBlock, Self::Error> {
		self.get_block_header_hash(hash).await.map_err(APIError::from_err)
	}
	...
	async fn send_to_address_send_token(
		...
		self.send_to_address_send_token(send_token).await.map_err(APIError::from_err)
	}
	...
	async fn send_from_send_token(
		...
		self.send_from_send_token(send_token, from).await.map_err(APIError::from_err)
	}
}
