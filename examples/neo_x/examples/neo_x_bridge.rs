...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Preparing to deposit {} GAS to Neo X address: {}", amount as f64 / 100_000_000.0, neo_x_address);
    
    // Build the deposit transaction
    // Note: In a real scenario, you would actually send this transaction
    let deposit_builder = bridge.deposit(
        &gas_token,
        amount,
        neo_x_address,
        &account,
    ...
    println!("Preparing to withdraw {} GAS to Neo N3 address: {}", amount as f64 / 100_000_000.0, neo_n3_address);
    
    // Build the withdraw transaction
    // Note: In a real scenario, you would actually send this transaction
    let withdraw_builder = bridge.withdraw(
        &gas_token,
        amount,
        neo_n3_address,
        &account,
