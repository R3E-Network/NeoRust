use anyhow::Result;
use neo3::prelude::*;
use neo3::fs::{NeoFSClient, NeoFSConfig, NeoFSAuth, Container, Object, AccessPermission};
use std::path::Path;
                        ...
                        match client.list_objects(&container_id).await {
                            Ok(object_ids) => {
                                ...
                                match client.get_object(&container_id, &object_id).await {
                                    Ok(retrieved_object) => {
                                        println!("Object downloaded (size: {} bytes)", retrieved_object.size());
                                        ...
                                        match client.create_bearer_token(&container_id, permissions, 3600).await {
                                            Ok(token) => {
                                                println!("Bearer token created: {}", token.token_id);
                                                
                                                // Delete object
                                                println!("Deleting object...");
                                                match client.delete_object(&container_id, &object_id).await {
                                                    Ok(true) => println!("Object deleted successfully"),
                                                    Ok(false) => println!("Failed to delete object"),
                                                ...
                                                match client.delete_container(&container_id).await {
                                                    Ok(true) => println!("Container deleted successfully"),
                                                    Ok(false) => println!("Failed to delete container"),
                                                    Err(e) => println!("Error deleting container: {}", e),
                                                }
                                            },
                                            Err(e) => println!("Error creating bearer token: {}", e),
                                        }
                                    },
