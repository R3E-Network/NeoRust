...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let swap_tx = flamingo.swap(&neo_token, &gas_token, amount, min_return, &account).await?;
	...
	let add_liquidity_tx = flamingo
		.add_liquidity(&neo_token, &gas_token, amount_neo, amount_gas, &account)
		.await?;
	...
	let remove_liquidity_tx = flamingo
		.remove_liquidity(&neo_token, &gas_token, liquidity_amount, &account)
		.await?;
	...
	let stake_tx = flamingo.stake(&lp_token, stake_amount, &account).await?;
