...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let deposit_tx = neocompound.deposit(&gas_token, amount, &account).await?;
	...
	let compound_tx = neocompound.compound(&gas_token, &account).await?;
	...
	let withdraw_tx = neocompound.withdraw(&gas_token, amount, &account).await?;
