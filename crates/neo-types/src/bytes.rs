...
impl Bytes {
    ...
    /// Converts the bytes to a BigInt
    ...
    /// Encodes the bytes as a base58 string
    pub fn base58_encoded(&self) -> String {
        bs58_encode(self.0.as_slice()).into_string()
    }

    /// Encodes the bytes as a base58check string
    ...
    /// Returns the variable size of the bytes
    ...
    /// Pads the bytes to the specified length
...
// Implement From<Vec<u8>> for Bytes
